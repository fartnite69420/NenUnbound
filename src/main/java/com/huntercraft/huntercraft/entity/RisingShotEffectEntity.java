...
public class RisingShotEffectEntity extends Entity {
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(RisingShotEffectEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(RisingShotEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(RisingShotEffectEntity.class, EntityDataSerializers.INT);

    public RisingShotEffectEntity(EntityType<? extends RisingShotEffectEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, float scale, int lifeTicks) {
        RisingShotEffectEntity entity = HunterEntityTypes.RISING_SHOT_EFFECT.get().create(level);
        if (entity == null) {
            return;
        }
