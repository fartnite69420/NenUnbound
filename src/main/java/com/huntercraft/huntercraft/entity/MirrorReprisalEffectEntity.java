...
public class MirrorReprisalEffectEntity extends Entity {
    public static final int MODE_GUARD = 0;
    public static final int MODE_STRIKE = 1;

    private static final EntityDataAccessor<Integer> MODE = SynchedEntityData.defineId(MirrorReprisalEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(MirrorReprisalEffectEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(MirrorReprisalEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(MirrorReprisalEffectEntity.class, EntityDataSerializers.INT);

    public MirrorReprisalEffectEntity(EntityType<? extends MirrorReprisalEffectEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
        this.setNoGravity(true);
    }

    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, int mode, float scale, int lifeTicks) {
        MirrorReprisalEffectEntity entity = HunterEntityTypes.MIRROR_REPRISAL_EFFECT.get().create(level);
        if (entity == null) {
            return;
        }
    ...
    protected void defineSynchedData() {
        this.entityData.define(MODE, MODE_GUARD);
