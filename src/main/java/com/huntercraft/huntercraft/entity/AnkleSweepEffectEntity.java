...
public class AnkleSweepEffectEntity extends Entity {
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(AnkleSweepEffectEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(AnkleSweepEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(AnkleSweepEffectEntity.class, EntityDataSerializers.INT);

    public AnkleSweepEffectEntity(EntityType<? extends AnkleSweepEffectEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, float scale, int lifeTicks) {
        AnkleSweepEffectEntity entity = HunterEntityTypes.ANKLE_SWEEP_EFFECT.get().create(level);
        if (entity == null) {
            return;
        }
