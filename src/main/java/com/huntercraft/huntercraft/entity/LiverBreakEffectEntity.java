...
public class LiverBreakEffectEntity extends Entity {
    public static final int STYLE_GUARD = 0;
    public static final int STYLE_IMPACT = 1;

    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(LiverBreakEffectEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(LiverBreakEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(LiverBreakEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> STYLE = SynchedEntityData.defineId(LiverBreakEffectEntity.class, EntityDataSerializers.INT);

    public LiverBreakEffectEntity(EntityType<? extends LiverBreakEffectEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, float scale, int lifeTicks, int style) {
        LiverBreakEffectEntity entity = HunterEntityTypes.LIVER_BREAK_EFFECT.get().create(level);
        if (entity == null) {
            return;
        }
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(STYLE, STYLE_IMPACT);
    }
    ...
    private void setStyle(int style) {
        this.entityData.set(STYLE, style == STYLE_GUARD ? STYLE_GUARD : STYLE_IMPACT);
    }
