...
public class ParrySparkEffectEntity extends Entity {
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(ParrySparkEffectEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(ParrySparkEffectEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(ParrySparkEffectEntity.class, EntityDataSerializers.INT);

    public ParrySparkEffectEntity(EntityType<? extends ParrySparkEffectEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, float scale, int lifeTicks) {
        ParrySparkEffectEntity entity = HunterEntityTypes.PARRY_SPARK_EFFECT.get().create(level);
        if (entity == null) {
            return;
        }
