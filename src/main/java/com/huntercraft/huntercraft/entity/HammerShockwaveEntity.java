...
public class HammerShockwaveEntity extends Entity {
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(HammerShockwaveEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> LIFE_TICKS = SynchedEntityData.defineId(HammerShockwaveEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> MAX_LIFE_TICKS = SynchedEntityData.defineId(HammerShockwaveEntity.class, EntityDataSerializers.INT);

    public HammerShockwaveEntity(EntityType<? extends HammerShockwaveEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public static void spawn(ServerLevel level, Vec3 position, Vec3 direction, float scale, int lifeTicks) {
        HammerShockwaveEntity entity = HunterEntityTypes.HAMMER_SHOCKWAVE.get().create(level);
        if (entity == null) {
            return;
        }
